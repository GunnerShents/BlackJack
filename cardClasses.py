...
class Card():
    ...
    def __init__(self, num, suit):
        ...
        self.value = {'Ace':11,'Two':2,'Three':3,'Four':4,'Five':5,'Six':6,
...
class CardImages():
    ...
    def __init__(self):
        #load images
        blank_card = pygame.image.load(path.join(config.IMG_DIR,"BJ_blankCard.png")).convert_alpha()
    ...
    def create_suits_dict(self):
        ...
        for image in os.listdir(suits_path):
            ...
            self.suits[key_entry.capitalize()+"s"] = pygame.image.load(path.join(suits_path, image)).convert_alpha()
    ...
    def num_dict(self, colour, num_dict):
        ...
        for image in os.listdir(num_path):
            ...
            if image[0] == colour:
                key_entry = path.splitext(image)[0].split("_")[1]
                num_dict[key_entry.capitalize()] = pygame.image.load(path.join(num_path, image)).convert_alpha()
...
class Deck():
    ...
    def __init__(self):
        ...
        self.numbers = ["Ace","Two","Three","Four","Five","Six","Seven","Eight","Nine"
                        ,"Ten","Jack","Queen","King"]
        self.deck = []
